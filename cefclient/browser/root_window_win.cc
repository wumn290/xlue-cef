...
void RootWindowWin::CreateRootWindow(const CefBrowserSettings& settings) {
  ...
  // Show this window.
  HWND hwnd = ::FindWindowA("xlue-cef-{37408185-9D8A-46be-968A-144FEBDB64D6}", "");
  if (hwnd == NULL){
	  MessageBoxA(0, "未找到xlue消息窗口", "提示", 0);
  }
  else{
	  DWORD dwStyleOld = GetWindowLong(hwnd_, GWL_STYLE);//获取旧样式  
	  DWORD dwNewStyle = WS_OVERLAPPED | WS_VISIBLE | WS_SYSMENU | WS_MINIMIZEBOX | WS_MAXIMIZEBOX | WS_CLIPCHILDREN | WS_CLIPSIBLINGS;  
	  dwNewStyle &= dwStyleOld;//按位与将旧样式去掉  
	  SetWindowLong(hwnd_, GWL_STYLE, dwNewStyle);//设置成新的样式  
	  SendMessage(hwnd, WM_USER+100, (WPARAM)hwnd_, 0);
  }
   Show(ShowNormal);
